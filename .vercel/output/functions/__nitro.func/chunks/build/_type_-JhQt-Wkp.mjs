...
import { withAsyncContext, ref, watchEffect, unref, computed, toValue, reactive, watch, getCurrentInstance, onServerPrefetch, shallowRef, toRef, nextTick, defineComponent, createElementBlock, provide, cloneVNode, h, useSSRContext } from 'vue';
...
const _sfc_main = {
  ...
  async setup(__props) {
    ...
    const { data, pending, error } = ([__temp, __restore] = withAsyncContext(() => useFetch(`https://pokeapi.co/api/v2/type/${type}`, "$NQKDstzsui")), __temp = await __temp, __restore(), __temp);
    const pokemons = ref([]);
    const loadingPokemons = ref(true);
    watchEffect(async () => {
      var _a;
      if ((_a = data.value) == null ? void 0 : _a.pokemon) {
        ...
        try {
          ...
          console.error("Erro ao carregar pok\xE9mons:", err);
        } finally {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(`<h1>Tipo: ${ssrInterpolate(unref(type))}</h1>`);
      if (unref(pending) || loadingPokemons.value) {
        _push(`<div>Carregando...</div>`);
      } else if (unref(error)) {
        _push(`<div>Erro: ${ssrInterpolate(unref(error).message)}</div>`);
      } else {
        _push(`<div>`);
        _push(ssrRenderComponent(_sfc_main$1, { pokemons: pokemons.value }, null, _parent));
        if (!pokemons.value.length) {
          _push(`<div>Nenhum Pok\xE9mon encontrado.</div>`);
        } else {
          _push(`<!---->`);
        }
        _push(`</div>`);
...
_sfc_main.setup = (props, ctx) => {
  ...
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("pages/pokemon/types/[type].vue");
  return _sfc_setup ? _sfc_setup(props, ctx) : void 0;
};

export { _sfc_main as default };
//# sourceMappingURL=_type_-JhQt-Wkp.mjs.map
